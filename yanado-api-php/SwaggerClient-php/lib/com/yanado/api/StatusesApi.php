...
class StatusesApi
{
     ...
     * @return \com.yanado.invoker\com.yanado.model\Status[]
     */
    public function getStatuses($list_id)
    {
        list($response) = $this->getStatusesWithHttpInfo($list_id);
        return $response;
    }
     ...
     * @param string $list_id List id (required)
     * @throws \com.yanado.invoker\ApiException on non-2xx response
     * @return array of \com.yanado.invoker\com.yanado.model\Status[], HTTP status code, HTTP response headers (array of strings)
     */
    public function getStatusesWithHttpInfo($list_id)
    {
        // verify the required parameter 'list_id' is set
        if ($list_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $list_id when calling getStatuses');
        }
        // parse inputs
        $resourcePath = "/public-api/lists/{listId}/statuses";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\com.yanado.invoker\com.yanado.model\Status[]',
                '/public-api/lists/{listId}/statuses'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\com.yanado.invoker\com.yanado.model\Status[]', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\com.yanado.invoker\com.yanado.model\Status[]', $e->getResponseHeaders());
